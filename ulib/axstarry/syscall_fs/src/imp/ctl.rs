...
pub fn syscall_renameat2(
    ...
    error!("old_path:{} new_path:{}", old_path.path(), new_path.path());
    // LAB3 从此处往上的代码不需要修改
    //
    // HINT 1
    ...
    if let Some(flag) = RenameFlags::from_bits(flags as u32) {
        if flag.contains(RenameFlags::RENAME_NOREPLACE) && path_exists(new_path.path()) {
            return Err(SyscallError::EPERM)
        }
        if flag.contains(RenameFlags::RENAME_EXCHANGE){
            let old_metadata = metadata(old_path.path()).unwrap();
            let new_metadata = metadata(new_path.path()).unwrap();
            if old_metadata.is_dir() != new_metadata.is_dir() {
                return Err(SyscallError::EPERM)
            }
        }
        if flag.contains(RenameFlags::RENAME_WHITEOUT) && flag.contains(RenameFlags::RENAME_EXCHANGE){
            return Err(SyscallError::EPERM)
        }
    }
    let rename = rename(old_path.path(), new_path.path());
    match rename {
        Ok(_) => Ok(0isize),
        Err(_) => {
            Err(SyscallError::EPERM)
